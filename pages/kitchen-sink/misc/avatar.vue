        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div v-for="user in users" :key="user.name" class="flex items-center gap-4 p-4 border border-surface-200 dark:border-surface-700 rounded-lg">
            <div class="relative">
              ...
              <div class="absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-white dark:border-surface-800" :class="getStatusColor(user.status)" />
            </div>
            <div class="flex-1 min-w-0">
              <h3 class="font-semibold text-surface-900 dark:text-surface-0 truncate">
                {{ user.name }}
              </h3>
              <p class="text-surface-600 dark:text-surface-300 text-sm truncate">
                {{ user.role }}
              </p>
              <p class="text-xs capitalize" :class="`text-${user.status === 'online' ? 'green' : user.status === 'away' ? 'yellow' : user.status === 'busy' ? 'red' : 'gray'}-600`">
                {{ user.status }}
              </p>
            </div>
          </div>
